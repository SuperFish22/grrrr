(function ($) {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                var pos = function (el, parent) {
                    ...
                    var bottomVal =
                        wrapper.outerHeight() -
                        (el.position().top + el.outerHeight());
                ...
                mEl.data().outerSizeFunc = function (el) {
                    if (
                        mEl.data().direction === "top" ||
                        mEl.data().direction === "bottom"
                    ) {
                ...
                var createMarquee = function () {
                    ...
                    if (mEl.data().outerSizeFunc(mMove) > 0) {
                        ...
                        var contentString = $("<div>")
                            .addClass("cloneContent")
                            .html(mMove.html());

                        if (
                            mEl.data().direction === "left" ||
                            mEl.data().direction === "right"
                        ) {
                            ...
                            if (mEl.outerHeight() === 0) {
                                alert(
                                    "Set Height Parametr for Plugin liMarquee"
                                );
                            }
                        ...
                        var cloneContent = function (mMove) {
                            if (mEl.data().outerSizeFunc(mMove) !== 0) {
                                ...
                                console.log(
                                    "The string is empty or contains invalid style"
                                );
                            }
                        };
                        ...
                        var anim = function (sPos, ePos) {
                            if (!mEl.data().pause) {
                                ...
                                if (ePos !== 0 && ePos !== -0) {
                                    ...
                                    var duration =
                                        (way * mEl.data().scrollDelay) /
                                        mEl.data().scrollAmount;
                                    var directTypeStart = {};
                                    var directTypeEnd = {};
                        ...
                        var addClone = function () {
                            ...
                            var cloneBefore = mMoveClone
                                .clone()
                                .addClass("cloneBefore")
                                .css(value)
                                .appendTo(mMove);
                            var cloneAfter = mMoveClone
                                .clone()
                                .addClass("cloneAfter")
                                .css(value2)
                                .appendTo(mMove);
                        ...
                        if (mEl.data().hoverStop) {
                            mEl.on(
                                "mouseenter." + mEl.data().mElIndex,
                                function () {
                                    mEl.off(
                                        "mouseleave." + mEl.data().mElIndex
                                    );
                        ...
                        if (
                            (!mEl.data().touchScreen &&
                                mEl.data().dragAndDrop) ||
                            (mEl.data().touchScreen && mEl.data().touchEvent)
                        ) {
                            mEl.on(mousedownEvent, function (e) {
                                ...
                                var startMouseCoord = correctEvent(e)[
                                    mEl.data().direction
                                ];
                                ...
                                $(document).on(moveEvent, function (e) {
                                    ...
                                    var newMouseCoord = correctEvent(e)[
                                        mEl.data().direction
                                    ];
                                    ...
                                    if (mEl.data().axis == "hor") {
                                        ...
                                        var vertDif = Math.abs(
                                            newMouseY - startMouseY
                                        );
                                        var horDif = Math.abs(
                                            newMouseX - startMouseX
                                        );
                                        vertSum += vertDif;
                                        horSum += horDif;
                                        if (vertSum > horSum) {
                                ...
                                if (
                                    !mEl.data()
                                        .touchScreen /* && !mEl.data().touchEvent*/
                                ) {
                                    return false;
                                }
                            });
                        ...
                        var getPosition = function (mEl) {
                            ...
                            if (mEl.data().circular) {
                                ...
                                mEl.data().endPos = endPos;
                                var circularPos = mEl.data().startShow
                                    ? mEl.data().outerSizeFunc(mMove)
                                    : mEl.data().outerSizeFunc(mEl) +
                                      mEl.data().outerSizeFunc(mMove);
                                anim(circularPos);
                            } else {
                                var tempStartPos = mEl.data().startShow
                                    ? 0
                                    : startPos;
                                anim(tempStartPos);
                            }
                        ...
                        var detectStringPos = function () {
                            if (mEl.data().stopOutScreen) {
                                if (
                                    mEl.offset().top + mEl.outerHeight() <
                                        $(window).scrollTop() ||
                                    mEl.offset().top >
                                        $(window).scrollTop() +
                                            $(window).height()
                    ...
                    $.ajax({
                        ...
                        success: function (xml) {
                            ...
                            for (var i = 0; i < xmlItemLength; i++) {
                                ...
                                var xmlItemContent = xmlItemActive
                                    .find("title")
                                    .text();
                                var xmlItemLink = xmlItemActive
                                    .find("link")
                                    .text();

                                if (xmlItemActive.find("link").length) {
                                    $(
                                        '<div class="mItem"><a href="' +
                                            xmlItemLink +
                                            '">' +
                                            xmlItemContent +
                                            "</a></div>"
    ...
    $.fn.liMarquee = function (method) {
        if (methods[method]) {
            ...
            $.error("Method " + method + " в jQuery.liMarquee doesn't exist");
        }
    };
})(jQuery);
